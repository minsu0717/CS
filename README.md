...
테스트 주도 개발에 대한 프로그래머들의 의견은 늘 엇갈린다.
...
디자인(설계) 단계에서 프로그래밍 목적을 반드시 미리 정의해야만 하고, 또 무엇을 테스트해야 할지 미리 정의(테스트 코드 작성)해야한다.
...
1. 디버깅 시간을 단축 할 수 있다.
  ...
  - 유닛 테스팅을 하는 이점이기도 하다. 예를 들어 사용자의 데이터가 잘못나오면 DB문제인지, 비즈니스 레이어 문제인지 전부 디버깅 해봐야 하지만, TDD의 경우 자동화 된 유닛테스팅을 전재하므로 특정 버그를 손 쉽게 찾아낼 수 있다.

2. 코드가 내손을 벗어나기 전에 가장 빠르게 피드백 받을 수 있다.
...
4. 재설계 시간을 단축 할 수 있다.
  ...
  - 테스트 코드를 먼저 작성하기 때문에 개발자가 지금 무엇을 해야하는지 분명히 정의 하고 개발을 하기 때문이다.
5. 추가 구현이 용이하다.

  - 자동화된 유닛 테스팅을 전제하므로 테스트 기간을 단축 시킬수 있다.
### TDD의 단점
1. 가장 큰 단점은 바로 생산성의 저하이다.
  - 처음부터 2개의 코드를 짜야하고, 중간중간 테스트를 하면서 고쳐나가야 하기 때문이다.

2. 이제까지 자신이 개발하던 방식을 많이 바꿔야 한다.

3. 구조에 얽매인다
  - 어려운 예외 상황이 생기면 그것 때문에 고민하는 순간이 찾아온다. 테스트는 테스트 일 뿐인데 원칙 때문에 쉽게 넘어가지 못하는 경우

### MVC 패턴
MVC는 사용자 인터페이스, 데이터 및 논리 제어를 구현하는데 널리 사용되는 소프트웨어 디자인 패턴이다. 소프트웨어의 비즈니스 로직과 화면을 구분하는데 중점을 두고 있으며, 이러한 관심사 분리는 더나은 업무의 분리와 향상된 관리를 제공한다.

결국 MVC패턴은 Model, View, Controller 라는 개념으로 역할을 나누어 일련의 작업을 처리하는 방법이다.

**Controller**

일종의 조정자라고 할 수 있다. 클라이언트의 요청을 받았을 때, 그 요청에 대해 실제 업무를 수행하는 모델 컴포넌트를 호출한다.또한 클라이언트가 보낸 데이터가 있다면, 모델에 전달하기 쉽게 데이터를 가공한다. 모델이 업무를 마치면 그 결과를 부에게 전달한다.
...
3. 서비스를 유지보수하고 테스트하는데 용이
4. 개발자 간의 커뮤니케이션 효율성을 높일 수 있다.

## MVC 패턴의 한계점

Model과 View 사이에는 controller를 통해 소통을 이루기에 의존성이 완전히 분리 될 수는 없다. 그래서 복잡한 대규모 프로그램의 경우 다수의 view와 model이 controller를 통해 연결되기 때문에 컨트롤러가 불필요하게 커지는 현상이 발생하는데 이러한 현상을 **Massive-View-Controller**현상 이라고 한다.
